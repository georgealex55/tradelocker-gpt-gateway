...
export async function buildRiskPreview(input) {
  ...
  const expectedAccountType = String(process.env.EXPECTED_ACCOUNT_TYPE || "").toLowerCase();

  if (expectedAccountType) {
    if (!actualAccountType) {
      fail("account_type", "Unable to determine TradeLocker account type");
    } else if (actualAccountType !== expectedAccountType) {
      fail(
        "account_type",
        `Connected account is ${actualAccountType}, expected ${expectedAccountType}`
      );
    } else {
      pass("account_type", actualAccountType);
    }
  } else if (actualAccountType) {
    pass("account_type", actualAccountType);
  }
