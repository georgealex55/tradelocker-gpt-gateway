...
function latestMarketTimestamp(raw) {
  const keys = new Set(["t", "time", "timestamp", "bartime", "bartimestamp"]);
  const found = [];
  ...
  function walk(value, parentKey = "") {
    ...
    if (Array.isArray(value)) {
      if (keys.has(String(parentKey).toLowerCase())) {
        for (const item of value) {
          const ts = normalizeEpochMs(item);
          if (ts) found.push(ts);
        }
      } else {
...
function configColumns(config, wantedKey) {
  function walk(obj) {
    ...
    for (const [k, v] of Object.entries(obj)) {
      if (k.toLowerCase() === wantedKey.toLowerCase()) return v;
    }
  ...
  const raw = walk(config);
  if (!raw) return [];

  // Shape 1: ["balance", "projectedBalance", ...]
  if (Array.isArray(raw)) {
    const withExplicitIndexes = raw
      .map((x, i) => {
        if (typeof x === "string") return { index: i, name: x };
        if (x && typeof x === "object") {
          const name = x.id || x.name || x.field || x.key || x.column || x.label || x.title || null;
          const idx = Number.isFinite(Number(x.index))
            ? Number(x.index)
            ...
            ? Number(x.id)
            : i;
          return name ? { index: idx, name } : null;
        }
        return null;
      })
      .filter(Boolean);

    if (!withExplicitIndexes.length) return [];
    const max = Math.max(...withExplicitIndexes.map(x => x.index));
    const columns = new Array(max + 1).fill(null);
    for (const x of withExplicitIndexes) columns[x.index] = x.name;
    return columns;
  }

  // Shape 2: { balance: 0, projectedBalance: 1, ... }
  if (raw && typeof raw === "object") {
    const entries = Object.entries(raw);

    const nameToIndex = entries
      .map(([name, idx]) => Number.isFinite(Number(idx)) ? { name, index: Number(idx) } : null)
      .filter(Boolean);

    if (nameToIndex.length) {
      const max = Math.max(...nameToIndex.map(x => x.index));
      const columns = new Array(max + 1).fill(null);
      for (const x of nameToIndex) columns[x.index] = x.name;
      return columns;
    }

    // Shape 3: { "0": "balance", "1": "projectedBalance", ... }
    const indexToName = entries
      .map(([idx, name]) =>
        Number.isFinite(Number(idx)) && typeof name === "string"
          ? { index: Number(idx), name }
          : null
      )
      .filter(Boolean);

    if (indexToName.length) {
      const max = Math.max(...indexToName.map(x => x.index));
      const columns = new Array(max + 1).fill(null);
      for (const x of indexToName) columns[x.index] = x.name;
      return columns;
    }

    // Shape 4: wrapper object with fields/columns/items.
    for (const key of ["fields", "columns", "items", "data"]) {
      if (raw[key]) return configColumns({ [wantedKey]: raw[key] }, wantedKey);
    }
  }
...
async function accountMetrics() {
  const [state, config, accountInfo] = await Promise.all([
    ...
    tlFetch("/trade/accounts").catch(() => null)
  ]);
  ...
  // Different TradeLocker servers expose equity under different names.
  // projectedBalance is commonly the closest current-equity equivalent.
  const equity =
    metric(readable, [
      "equity",
      "accountEquity",
...
export async function buildRiskPreview(input) {
  ...
  if (boolEnv("KILL_SWITCH", true)) fail("kill_switch", "KILL_SWITCH is enabled or unset");
  ...
  // Stale-market-data guard. TradeLocker quotes do not include a timestamp on
  // this broker, so validate the feed against the latest 1-minute history bar.
  let marketDataTimestamp = null;
  let marketDataAgeMs = null;
  const staleGuardEnabled = boolEnv("STALE_QUOTE_GUARD", true);

  if (staleGuardEnabled) {
    const nowMs = Date.now();
    const lookbackMs = numEnv("MARKET_FRESHNESS_LOOKBACK_MS", 15 * 60 * 1000);
    const maxAgeMs = numEnv("MAX_MARKET_DATA_AGE_MS", 3 * 60 * 1000);

    try {
      const historyRaw = await tlFetch(
        `/trade/history?routeId=${infoRouteId}&tradableInstrumentId=${tradableInstrumentId}&resolution=1m&from=${nowMs - lookbackMs}&to=${nowMs}`
      );

      marketDataTimestamp = latestMarketTimestamp(historyRaw);
      marketDataAgeMs =
        marketDataTimestamp != null ? Math.max(0, nowMs - marketDataTimestamp) : null;

      if (marketDataTimestamp == null) {
        fail("market_data_freshness", "No recent 1-minute market timestamp was returned");
      } else if (marketDataAgeMs > maxAgeMs) {
        fail(
          "market_data_freshness",
          `Latest market bar is ${Math.round(marketDataAgeMs / 1000)}s old; max is ${Math.round(maxAgeMs / 1000)}s`
        );
      } else {
        pass(
          "market_data_freshness",
          `${Math.round(marketDataAgeMs / 1000)}s old`
        );
      }
    } catch (e) {
      if (boolEnv("STRICT_STALE_QUOTE_CHECK", true)) {
        fail("market_data_freshness", `Freshness check failed: ${e.message}`);
      } else {
        warnings.push(`Market freshness check unavailable: ${e.message}`);
      }
    }
  }
  ...
  const accountList = acct?.accountInfo?.d;
  const currentAccount = Array.isArray(accountList)
    ? accountList.find(a => String(a.id) === String(accountId())) || accountList[0]
    : null;

  const actualAccountType = String(currentAccount?.type || "").toLowerCase();

  // Lock execution to one exact TradeLocker account. EXPECTED_ACCOUNT_ID is optional:
  // if omitted, the configured TRADELOCKER_ACCOUNT_ID becomes the lock target.
  const expectedAccountId = String(
    process.env.EXPECTED_ACCOUNT_ID || accountId()
  );
  const actualAccountId = currentAccount?.id != null
    ? String(currentAccount.id)
    : null;

  if (!actualAccountId) {
    fail("account_id_lock", "Unable to determine connected TradeLocker account ID");
  } else if (actualAccountId !== expectedAccountId) {
    fail(
      "account_id_lock",
      `Connected account ID ${actualAccountId} does not match locked account ID ${expectedAccountId}`
    );
  } else {
    pass("account_id_lock", actualAccountId);
  }

  // Keep TradeLocker's account type only as informational metadata because some
  // broker integrations label funded accounts inconsistently.
  if (actualAccountType) {
    pass("account_type_info", actualAccountType);
  }
  ...
  return {
    ...
    account: {
      ...
      type: actualAccountType || null,
      currency: currentAccount?.currency || accountCurrency,
      balance: acct.balance,
      projectedBalance: acct.equity
    },
    exposure: {
